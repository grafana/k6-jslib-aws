...
import { SQSClient, SQSServiceError, ReceivedMessage } from '../../dist/sqs.js'
...
export async function sqsTestSuite(data) {
    ...
    await asyncDescribe('sqs.deleteMessage with invalid handle', async (expect) => {
        // Act
        await sqsClient.deleteMessage(data.sqs.testQueues.standard, "handle")
            .then(
                ...
                (error) => {
                        ...
                        expect(error.message).to.equal("The input receipt handle \"handle\" is not a valid receipt handle.");
                    });
    })

    await asyncDescribe('sqs.deleteMessage', async (expect) => {
        // Arrange
        const queueMessages = await sqsClient.receiveMessages(data.sqs.testQueues.deleteQueue, undefined, undefined, undefined, undefined, 1);
        ...
        // Act
        await sqsClient.deleteMessage(data.sqs.testQueues.deleteQueue, gotFirstMessage.ReceiptHandle);
    })
    ...
    await asyncDescribe('sqs.receiveMessages no messages received', async (expect) => {
        // Act
        const messages = await sqsClient.receiveMessages(data.sqs.testQueues.emptyQueue, undefined, undefined, undefined, undefined, 1);

        // Assert
        expect(messages).to.have.length(0);
    })

    await asyncDescribe('sqs.receiveMessages receives only one message by default', async (expect) => {
        // Act
        const messages = await sqsClient.receiveMessages(data.sqs.testQueues.fifo, undefined, undefined, undefined, 0, 1);
    ...
    await asyncDescribe('sqs.receiveMessages up to ten messages', async (expect) => {
        // Act
        const messages = await sqsClient.receiveMessages(data.sqs.testQueues.fifo, ["ANY"], ["ANY"], 10, 0, 1);
    ...
    await asyncDescribe('sqs.receiveMessages from standard queue', async (expect) => {
        // Act
        const messages = await sqsClient.receiveMessages(data.sqs.testQueues.standard);
    ...
    await asyncDescribe('sqs.sendFIFOMessage', async (expect) => {
        ...
        // Act
        const message = await sqsClient.sendMessage(data.sqs.testQueues.fifoQueueSend,
            'test',
            { messageDeduplicationId: 'abc123', messageGroupId: 'easyasDoReMi',  })
