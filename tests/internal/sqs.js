...
export async function sqsTestSuite(data) {
    ...
    await asyncDescribe('sqs.deleteMessage with invalid handle', async (expect) => {
        // Act
        await sqsClient.deleteMessage(data.sqs.testQueues.standard, 'handle').then(
            () => {
    ...
    await asyncDescribe('sqs.sendFIFOMessage', async (expect) => {
        ...
        // Act
        const message = await sqsClient.sendMessage(data.sqs.testQueues.fifoQueueSend, 'test', {
